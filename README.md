...
webpages which are created, configured and contributed in [Jahia][jahia-website]. This application cannot run alone
without its backend content.

> Please be sur you have an environment up and running before to continue.
...
## Create a component
This section explains how to create a component. Before to start let's explain what you
will create.
...
### Objective
Create a component **MediaContentBloc** to render a content type defined in Jahia with the name
...
<img src="/doc/images/001_MediaBloc.png" width="400px"/>
...
Content types are managed in the backend with Jahia. Thus, to add a new content type you can
...
For this **MediaContentBloc** the definition already exists in the module [jahia-industrial] which is the
...
```
In the code above we have the minimal set of elements to start our component. The component is based on
the **HTML provided by the designer**. For the moment the component contains static content, so the next
step is to query the content dynamically.

#### Step 2 query the content from the Jahia CMS
Now you have your static HTML, you should request the content from the Jahia CMS. To do that, follow the
next steps:

- Import the **useNode** function to access easily the content properties. Note that **useNode** is a
wrapper function to simplify graphQL call.
    ```js
...
- Then, manage loading and error state.
    ...
    ```
- Finally, extract the properties returned into constants.
    ```js
    const {title,teaser,iconName} = data.properties;
    ```

So your file **MediaContentBloc.js** should look like this :
```js
import React from "react";
...
export default MediaContentBloc;
```
The next step is to replace the static content by the dynamic content returned by **useNode**.

#### Step 3 add the dynamic content into the HTML
First, you have to manage properly the icon. You get the icon name from the content, and based on this
...
- Import the components to manage icons.
    ...
    ```
- Write a function to use the appropriate icon component based on the iconName property.
    ```js
    const getIcon = () => {
        if (iconName) {
    ...
    ```
- Finally, replace the static content with dynamic properties.
    ```js
    <div className="media block-6 d-block text-center">
...
is used to merge OOTB components provided by the **Jahia SDK** and components you specifically
